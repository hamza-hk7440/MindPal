...
class SplitResourcesIntoChunksUseCase:
    ...
    async def execute(self, resource_id: UUID, study_subject_id: UUID) -> None:
        ...
        try:
            # resource.content is a Value Object; extract the raw string
            chunks_content = await self.slice_document_service.slice_document_into_chunks(resource.content.value)
        except Exception as e:
            raise ChunkingFailureException(f"Failed to chunk resource {resource_id}: {str(e)}")
        
        for content in chunks_content:
            # Create a Chunk through the domain factory (embedding empty for now)
            chunk = Chunk.create(source_id=resource.id, study_subject_id=study_subject_id, content=content, embedding=[])
            await self.chunks_repo.save_chunk(chunk)
            event = SplitChunkEvent(chunk_id=chunk.id, subject_id=study_subject_id, resource_id=resource_id)

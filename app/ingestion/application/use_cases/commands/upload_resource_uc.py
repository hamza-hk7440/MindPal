...
class UploadResourceUseCase:
    def __init__(
        ...
        extract_bytes_from_url_service: IExtraxtBytesFromUrlService,
    ...
    async def _extract_content(self, document_type: Doc_type, doc_url: str) -> Optional[str]:
        ...
        if document_type == Doc_type.PDF:
            pdf_bytes = await self.extract_bytes_from_url_service.extract_bytes_from_url(doc_url)
        ...
        if document_type == Doc_type.AUDIO:
            audio_bytes = await self.extract_bytes_from_url_service.extract_bytes_from_url(doc_url)
        ...
        if document_type == Doc_type.IMAGE:
            image_bytes = await self.extract_bytes_from_url_service.extract_bytes_from_url(doc_url)

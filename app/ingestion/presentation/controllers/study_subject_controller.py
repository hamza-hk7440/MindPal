...
class StudySubjectController:
    ...
    async def delete_study_subject(self, subject_id: UUID) -> None:
        ...
        try:
            ...
            print(f"Successfully deleted study subject with ID: {subject_id}")
    ...
    async def fetch_single_study_subject(self, subject_id: UUID) -> StudySubjectResponse:
        """
        Fetches a single study subject by its UUID.
        """
        try:
            study_subject_dto = await self._fetch_uc.fetch_one_study_subject(subject_id=subject_id)
            return StudySubjectResponse(
                id=study_subject_dto.id,
                user_id=study_subject_dto.user_id,
                name=study_subject_dto.name.value if hasattr(study_subject_dto.name, 'value') else study_subject_dto.name,
                created_at=study_subject_dto.created_at
            )
        except ResourceNotFoundException as exc:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Study subject not found: {str(exc)}"
            )